...
The disposable realm grants broad management scope only to make its generated import portable; production must map and review least-privilege user-management roles. Production email/Keycloak infrastructure, secrets, alerts, identity-disable synchronization, failure reconciliation and recovery approval remain gates. This slice includes no employee provider provisioning or activation, membership administration, last-owner protection, company setup UI or customer security-audit UI. A subsequent slice now records protected pending employee-account requests only.

Next P01-02 work is provider reconciliation, setup delivery and activation for the now-implemented [employee-account request](employee-account-requests.md), followed by membership administration and last-owner protection. See [authentication operations](../../operations/authentication.md), [Keycloak operations](../../operations/keycloak.md) and the [Phase 1 specification](../../implementation/PHASE-01-PLATFORM-PEOPLE.md).
