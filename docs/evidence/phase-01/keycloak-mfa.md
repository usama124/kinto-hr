...
Subsequent increments locally verify protected [initial-owner](owner-invitations.md) and [employee account](employee-account-activation.md) provider setup/activation plus [owner membership administration and last-owner protection](membership-administration.md). Administrator invitations, tenant selection and customer security audit are still missing. Authentication remains disabled by default; no employee CRUD API has been opened. See [operations](../../operations/keycloak.md) and the [roadmap](../../implementation/README.md).
