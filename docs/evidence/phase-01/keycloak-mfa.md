...
Provider password reset still does **not** notify Kinto to revoke every existing opaque session. Recovery is therefore not production-ready. Identity-disable/logout back-channel handling, credential-change session invalidation, production email/HTTPS/provider database, MFA recovery, edge abuse controls, key rotation and outage tests remain gates.

Protected first-operator/company-owner provisioning, company-admin/HR employee invitations and activation, membership administration/selection, last-owner protection and security audit are still missing. Authentication remains disabled by default; no employee/business API has been opened. Next engineering work is credential-change revocation, then administrator-only provisioning. See [operations](../../operations/keycloak.md) and the [roadmap](../../implementation/README.md).
