...
Subsequent increments locally verify protected [initial-owner](owner-invitations.md) and [employee account](employee-account-activation.md) provider setup/activation. Membership administration/selection, last-owner protection and security audit are still missing. Authentication remains disabled by default; no employee CRUD API has been opened. See [operations](../../operations/keycloak.md) and the [roadmap](../../implementation/README.md).
