...
Subsequent increments locally verify protected [initial-owner](owner-invitations.md), [employee account](employee-account-activation.md) and [administrator invitation](administrator-invitations.md) provider setup/activation plus [owner membership administration and last-owner protection](membership-administration.md) and [server-side tenant selection](tenant-selection.md). Customer security audit is still missing. Authentication remains disabled by default; no employee CRUD API has been opened. See [operations](../../operations/keycloak.md) and the [roadmap](../../implementation/README.md).
