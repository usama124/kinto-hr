...
The subsequent [initial-owner invitation increment](owner-invitations.md) locally verifies protected provider setup and first-owner activation. Company-admin/HR employee invitations, membership administration/selection, last-owner protection and security audit are still missing. Authentication remains disabled by default; no employee API has been opened. See [operations](../../operations/keycloak.md) and the [roadmap](../../implementation/README.md).
