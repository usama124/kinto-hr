...
Next, a restricted provider-provisioning worker must create or reconcile the named account without exposing a password, then issue a digest-only, expiring, single-use invitation bound to the intended verified identity. Activation must atomically grant the first owner and make replay, wrong-identity, expiry, revocation and provider failure deny access. Company/HR employee-account provisioning, membership administration and last-owner protection remain pending.
