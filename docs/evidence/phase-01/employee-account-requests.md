...
The subsequent [employee activation increment](employee-account-activation.md) implements exact provider reconciliation, expiring setup delivery and trusted-MFA activation that links one provider identity, one employee and one fixed `employee` membership without affecting owner/HR roles. [Owner membership administration and last-owner protection](membership-administration.md) and [administrator invitations](administrator-invitations.md) are also locally implemented. Tenant selection, security-audit UI and production delivery/reconciliation remain later gates.
