...
Latest bounded P01-02 increment: [session-revocation evidence](../evidence/phase-01/session-revocation.md) adds signed OIDC back-channel logout, atomic Redis subject/provider-session deletion and real Keycloak password-reset revocation on top of [Keycloak MFA/recovery](../evidence/phase-01/keycloak-mfa.md), [OIDC/session authentication](../evidence/phase-01/authentication.md) and [membership access](../evidence/phase-01/membership-access.md). Authentication remains disabled by default and only accepts already-provisioned identities. Generic providers/claims receive no MFA trust. Production callback failure reconciliation, reset-policy enforcement and identity-disable synchronization remain recovery gates. No employee HTTP endpoint is exposed. Next is protected administrator-led account provisioning/invitations, then membership administration and company setup.

Account provisioning clarification: [C11–C12](DECISIONS.md) exclude company and employee self-signup in all phases and plans. Platform admins create companies/initial owners; company admins/authorized HR create employee accounts within their tenant. The login/session adapter is implemented; administrator provisioning, invitation activation and password recovery remain unimplemented. Recovery must not create or restore revoked access. The Phase 1 acceptance checklist includes these authorization and recovery tests.
