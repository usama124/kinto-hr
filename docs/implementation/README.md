...
Latest bounded P01-02 increment: [company-provisioning evidence](../evidence/phase-01/company-provisioning.md) adds separate platform authority, explicit first-operator bootstrap and an MFA/CSRF-protected idempotent company plus pending-owner request. It builds on [session revocation](../evidence/phase-01/session-revocation.md), [Keycloak MFA/recovery](../evidence/phase-01/keycloak-mfa.md), [OIDC/session authentication](../evidence/phase-01/authentication.md) and [membership access](../evidence/phase-01/membership-access.md). Authentication remains disabled by default and only accepts already-provisioned identities. The new request creates no owner identity or membership until provider/invitation success. Production callback reconciliation, reset-policy enforcement and identity-disable synchronization remain recovery gates. No employee HTTP endpoint is exposed. Next is provider reconciliation and initial-owner invitation activation, then employee provisioning and membership administration.

Account provisioning clarification: [C11–C12](DECISIONS.md) exclude company and employee self-signup in all phases and plans. Platform admins create companies/initial owners; company admins/authorized HR create employee accounts within their tenant. The company-request boundary is implemented, while provider-side account creation, invitation activation and employee provisioning remain incomplete. The local provider password-recovery path is verified but not approved for production. Recovery must not create or restore revoked access. The Phase 1 acceptance checklist includes these authorization and recovery tests.
...
The first delivery is the safe **P00-04 / P01-01 foundation slice**, with evidence in [the foundation record](../evidence/phase-01/foundation.md). Subsequent P01-01 slices add the local worker/runtime, migration regression coverage, synthetic database restore drill and private worker monitoring; see [worker evidence](../evidence/phase-01/worker.md), [recovery/monitoring evidence](../evidence/phase-01/recovery-monitoring.md) and [operating procedures](../operations/foundation.md). Foundation is not complete: next engineering work is P01-02 owner provider/invitation completion, employee provisioning, membership administration and company/policy setup before employee endpoints are exposed. Provider callback reconciliation remains a production recovery gate. Deployed backup/PITR/file recovery, notification routing, approved staging and remote CI evidence remain operational gates. P01-03 entitlements, P01-04 full employee/compensation workflows and P01-05 imports/private files remain required for the full phase. P00-01 still needs owner-approved pilot details; K50 hardware and payroll review gates remain external dependencies.
