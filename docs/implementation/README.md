...
Latest bounded P01-02 increment: [OIDC/session authentication](../evidence/phase-01/authentication.md) adds optional login/callback/session/logout routes, Redis server sessions and an account-access page on top of [membership access](../evidence/phase-01/membership-access.md). Authentication is disabled by default and only accepts already-provisioned identities. Provider-specific MFA is deliberately unverified, so privileged operations stay denied. No employee HTTP endpoint is exposed. Next is real-provider MFA/recovery and protected administrator-led account provisioning/invitations, then membership administration and company setup.
