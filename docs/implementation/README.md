...
Latest bounded P01-03 increment: [base-entitlement evidence](../evidence/phase-01/base-entitlements.md) adds immutable price-free plan versions, one base subscription during platform provisioning, legacy-capacity backfill, a safe owner/HR effective projection and subscribed capacity enforcement under the existing tenant lock. Complimentary mode suppresses collection without changing role permission. Only the implemented company-setup capability is present. Dated grants, add-ons, field overrides, preview/revocation and invalidation remain the next P01-03 slice.
