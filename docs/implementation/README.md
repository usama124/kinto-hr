...
Latest bounded P01-02 increment: [initial-owner invitation evidence](../evidence/phase-01/owner-invitations.md) adds optional Keycloak exact-email reconciliation, provider-managed 48-hour setup delivery and atomic exact-subject trusted-MFA first-owner activation. It builds on the [company request](../evidence/phase-01/company-provisioning.md), [session revocation](../evidence/phase-01/session-revocation.md), [Keycloak MFA/recovery](../evidence/phase-01/keycloak-mfa.md), [OIDC/session authentication](../evidence/phase-01/authentication.md) and [membership access](../evidence/phase-01/membership-access.md). Authentication and provisioning remain disabled by default. Provider failure creates no membership. Production delivery/reconciliation, reset-policy enforcement and identity-disable synchronization remain recovery gates. No employee HTTP endpoint is exposed. Next is employee provisioning and membership administration.

Account provisioning clarification: [C11–C12](DECISIONS.md) exclude company and employee self-signup in all phases and plans. Platform admins create companies/initial owners; company admins/authorized HR create employee accounts within their tenant. The company-request and initial-owner activation boundaries are locally implemented; employee provisioning remains incomplete. The local provider invitation/recovery paths are verified but not approved for production. Recovery must not create or restore revoked access. The Phase 1 acceptance checklist includes these authorization and recovery tests.
