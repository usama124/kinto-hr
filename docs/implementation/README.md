...
Latest bounded P01-02 increment: [Keycloak MFA/recovery evidence](../evidence/phase-01/keycloak-mfa.md) adds an explicit LoA 2 trust profile and a pinned real-provider browser/TOTP/reset-email workflow on top of [OIDC/session authentication](../evidence/phase-01/authentication.md) and [membership access](../evidence/phase-01/membership-access.md). Authentication remains disabled by default and only accepts already-provisioned identities. Generic providers/claims receive no MFA trust. Provider password reset is verified, but complete Kinto session revocation on credential changes is pending, so recovery is not production-ready. No employee HTTP endpoint is exposed. Next is credential-change revocation and protected administrator-led account provisioning/invitations, then membership administration and company setup.
...
The first delivery is the safe **P00-04 / P01-01 foundation slice**, with evidence in [the foundation record](../evidence/phase-01/foundation.md). Subsequent P01-01 slices add the local worker/runtime, migration regression coverage, synthetic database restore drill and private worker monitoring; see [worker evidence](../evidence/phase-01/worker.md), [recovery/monitoring evidence](../evidence/phase-01/recovery-monitoring.md) and [operating procedures](../operations/foundation.md). Foundation is not complete: next engineering work is P01-02 provider credential-change revocation, protected provisioning/invitations, membership administration and company/policy setup before employee endpoints are exposed. Deployed backup/PITR/file recovery, notification routing, approved staging and remote CI evidence remain operational gates. P01-03 entitlements, P01-04 full employee/compensation workflows and P01-05 imports/private files remain required for the full phase. P00-01 still needs owner-approved pilot details; K50 hardware and payroll review gates remain external dependencies.
