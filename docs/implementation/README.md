...
Latest bounded P01-02 increment: [employee-account request evidence](../evidence/phase-01/employee-account-requests.md) adds a recent-MFA, tenant-scoped owner/HR boundary for one idempotent pending account request per existing employee. It builds on [initial-owner activation](../evidence/phase-01/owner-invitations.md), the [company request](../evidence/phase-01/company-provisioning.md), [OIDC/session authentication](../evidence/phase-01/authentication.md) and [membership access](../evidence/phase-01/membership-access.md). It fixes the intended future role to employee and creates no provider identity or membership. Authentication/provider provisioning remain disabled by default. Next is employee provider delivery/activation and membership administration.

Account provisioning clarification: [C11–C12](DECISIONS.md) exclude company and employee self-signup in all phases and plans. Platform admins create companies/initial owners; company admins/authorized HR create employee accounts within their tenant. The company/employee request and initial-owner activation boundaries are locally implemented; employee provider activation remains incomplete. The local provider invitation/recovery paths are verified but not approved for production. Recovery must not create or restore revoked access. The Phase 1 acceptance checklist includes these authorization and recovery tests.
