...
P01-04 is locally implemented through [basic onboarding/offboarding checklists](../evidence/phase-01/employee-checklists.md), including an automatic final-settlement review task when termination is scheduled. P01-05 has started with a [digest-backed employee CSV validation preview](../evidence/phase-01/employee-import-preview.md); it creates no employees until the separate confirmation boundary is implemented. Documents and self-service remain.
