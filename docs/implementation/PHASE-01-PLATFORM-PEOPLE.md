...
- Migration evidence: thirty-five migrations are applied locally; isolated previous-schema upgrade/backfill, explicit first-operator bootstrap/replay, two-tenant delivery isolation and migration replay are verified. Local synthetic database restore includes identity/organization/catalog/employee-assignment history, committed employee imports, pending contact-change requests, document quarantine metadata, checklists, future termination metadata, archive/rehire history and entitlements; fresh-cluster/PITR/file recovery and staging evidence remain pending.
