...
The company owner/HR admin are primary users. Employee self-service is limited to authorized profile viewing/change requests and employee-visible documents. Platform operators manage tenant capabilities without access to HR content.
...
- Absolute `/api/v1/platform/tenants` platform-admin-only company/initial-owner provisioning; `/api/v1/platform/tenants/{id}/entitlement-changes` previews, creates and revokes bounded entitlement controls, never HR data. Commercial metadata listing remains future work.
...
- `/employees/{id}/checklist` permission-protected list/create and `/employees/{id}/checklist/{taskId}/complete` versioned completion commands. Task assignee and completion actor come from active tenant identities; clients cannot set completion evidence.
...
- Work packages: P01-01 partially implemented; the P01-02 application boundary is locally implemented with production provider/recovery gates still open; P01-03 base plans and dated entitlement controls are locally implemented; P01-04 organization catalogs, public employee lifecycle through capacity-backed rehire, restricted contact/emergency/address/CNIC details, effective-dated compensation and basic onboarding/offboarding checklists are locally implemented. P01-05 includes the [employee CSV validation and atomic confirmation workflow](../evidence/phase-01/employee-import-preview.md) and document quarantine metadata and an off-by-default [local-test upload/scan/HR-and-employee-download path](../evidence/phase-01/employee-documents.md) and [profile-change employee/HR workspace](../evidence/phase-01/employee-profile-change-requests.md), plus a tenant-safe [workforce headcount report snapshot](../evidence/phase-01/workforce-headcount-report.md); production object transfer/scanning/download, file recovery, audited CSV exports and the report workspace remain.
...
- P01-02 authentication increment: optional OIDC code/PKCE/state/nonce/RS256 validation, Redis server sessions, logout/CSRF and account-access UI are implemented; see [authentication evidence](../evidence/phase-01/authentication.md). A pinned real Keycloak LoA 2/TOTP, setup/reset email and signed back-channel workflow is locally verified; see [provider evidence](../evidence/phase-01/keycloak-mfa.md), [revocation evidence](../evidence/phase-01/session-revocation.md) and [owner-invitation evidence](../evidence/phase-01/owner-invitations.md). The protected control plane separates platform authority, bootstraps only the explicit first operator, atomically creates an idempotent company request, optionally reconciles/delivers its exact initial owner, and grants the first owner only after exact-subject trusted MFA. Owners/authorized HR can record an idempotent tenant-scoped employee account request; the optional provider path delivers setup and atomically creates only its fixed employee membership/link after exact-identity trusted MFA. Recent-MFA owners can list memberships, make audited/versioned administrative role changes and invite another owner/HR/payroll administrator with a fixed audited role set. Disabled-first provider delivery and exact-subject MFA atomically create the invited membership; HR, employee/platform roles, existing same-tenant access and overlapping invitation types are refused. Active-company discovery and server-side selection bind every tenant-scoped HTTP path to the selected session context; see [tenant-selection evidence](../evidence/phase-01/tenant-selection.md). Recent-MFA owners can review a safely projected tenant audit feed with strict filters and opaque keyset pagination while platform audit stays separate; see [security-audit evidence](../evidence/phase-01/security-audit.md). Authentication and provider provisioning are disabled by default.
...
- Acceptance: no full Phase 1 acceptance. P01-01 local engineering includes database recovery and worker monitoring; deployed backup/PITR/file recovery, alert notification delivery and staging review remain. P01-02 is locally verified as recorded in [organization-policy evidence](../evidence/phase-01/organization-policies.md). P01-03 is locally verified in [base-entitlement evidence](../evidence/phase-01/base-entitlements.md) and [entitlement-control evidence](../evidence/phase-01/entitlement-controls.md). P01-04 is locally implemented through organization catalogs, employee lifecycle/rehire, private details, compensation and [basic checklist evidence](../evidence/phase-01/employee-checklists.md). P01-05 remains. No invoice or payroll endpoint is exposed.
- Migration evidence: thirty-seven migrations are applied locally; isolated previous-schema upgrade/backfill, explicit first-operator bootstrap/replay, two-tenant delivery isolation and migration replay are verified. Local synthetic database restore includes identity/organization/catalog/employee-assignment history, committed employee imports, contact-change decisions, document quarantine metadata, checklists, future termination metadata, archive/rehire history and entitlements; fresh-cluster/PITR/file recovery and staging evidence remain pending.
