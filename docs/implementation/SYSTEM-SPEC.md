...
### Company-wide business policy administration

Each tenant owns a central Company Policies area for supported organization, attendance/timing, leave, absence-settlement and payroll settings. The published company policy applies across its employees and branches. V1 has no hidden personal or branch policy overrides; employee salary agreements remain individual data, and explicit assignments to company-managed named shifts select published timing definitions rather than bypassing the company's rules. Employee eligibility and joining-date proration follow the same company policy rather than a manually invented personal quota.

Persist typed domain policy versions and effective dates behind one administration interface, not an arbitrary customer scripting engine. Require `company_policies.read`/`company_policies.manage`, plus the relevant payroll permission for payroll-sensitive settings. Allow only implemented, schema-validated settings to be activated. Provide draft, preview and publish actions, reason/actor audit and version-conflict handling. Reject overlapping effective versions in the same policy domain; cache by tenant/domain/version and invalidate on publication. Resolve the version by the affected work/leave date or explicitly versioned payroll period, never merely by today's settings.

Preview names affected dates and unresolved/open periods without exposing unauthorized salary data. Publishing may recalculate affected open attendance and invalidate unfinalized payroll inputs/approvals, but cannot mutate locked attendance, leave consumption history or finalized payroll. Retain the exact timing, quota, settlement and salary-calculation policy versions in snapshots and reports. Conflicts with existing future leave reservations become visible exceptions, not silent cancellation. Tenant isolation, access controls, review requirements, maximum supported capacities and applicable statutory safeguards cannot be disabled by a company setting.
...
Leave submissions also enforce per-employee/type annual entitlement and monthly usage limits on the server, including requests entered by HR. Pending reservations and approved consumption count once against both periods; all dates in a cross-month/year request reserve atomically or none do. Monthly cap errors are distinct from annual balance errors. Policy versions cannot reset usage, and the leave application date cannot move consumption into another month's bucket.

## 5. Jobs, events and consistency

Confirmed workflow constraints: the K50 collector is a local LAN service with outbound HTTPS, durable buffering and transport/source deduplication; cloud ingestion must never count a retried or re-polled source event twice. Salary payments occur outside Kinto. Scheduled payroll creates a reviewable run/report and cannot authorize money movement or bypass payroll approvals. Employee compensation is captured in Phase 1 and reused by Phase 3, with explicit `payroll.compensation.read`/`payroll.compensation.write` permissions assignable to designated company/HR staff; ordinary employee directory access does not expose salary.
...
Monthly payroll schedules are persisted, versioned and evaluated in the employer's configured timezone, with generation date distinct from covered period and external pay date. Regular-run uniqueness is employer/period based, not queue-job or schedule-version based. After outage/retry, resume the same due occurrence. Missing inputs create observable blockers, not zero-pay results. Recheck schedule authorization and entitlements before calculation, and require a human preparer plus separate approver for finalization. See P03-02 for period/default, catch-up and short-month rules; they are specification requirements, not implemented scheduler behavior.

Absence deductions use locked, reviewed attendance/leave segments, with approved paid leave, policy-covered Annual leave, approved unpaid leave and residual uncovered absence distinct. Company-wide absence policy selects salary deduction or Annual-leave-first with an explicit insufficient-quota fallback. Settle Annual quota atomically before attendance lock, using the ordinary annual/monthly usage locks; payroll only reads that outcome and never spends quota again. Pending requests or device uncertainty cannot be silently resolved into a salary deduction. Persist the employer-reviewed deduction base/divisor and each source quantity/rate in the calculation snapshot; never deduct the same unpaid segment twice. Payroll UI/CSV/PDF report the attendance/leave breakdown, salary entitlement, itemized deductions and net payable, not a claim of completed salary transfer.
