...
**Recovery is not production-ready.** Invalidating every existing Kinto session after a provider password reset is still required; the current opaque-session adapter does not receive provider credential-change/logout events. Do not equate a tested provider email flow with complete Kinto recovery. Provider/all-session revocation, identity-disable synchronization and MFA recovery need implementation and tests before live use.
