...
This increment has no company-provisioning UI or employee account workflow. Initial-owner creation is available only through the protected platform API and remains administrator-only under C11–C12. Employee provisioning, membership administration, last-owner protection and customer-visible security audit are still required. Production Keycloak database/backups, least-privilege service-account configuration, HTTPS, email delivery/reconciliation, abuse protection, secrets, availability and security review remain unapproved. `AUTH_MODE=disabled` and `ACCOUNT_PROVISIONING_MODE=disabled` stay the defaults.
