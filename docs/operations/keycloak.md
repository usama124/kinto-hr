...
This increment has no company-provisioning UI or employee CRUD workflow. Initial-owner creation is available only through the protected platform API; employee login requests are available only to a same-company owner/authorized HR under C11–C12. Owner membership administration, last-owner protection, owner-created administrator invitations, server-side active-company selection and owner-only tenant security-audit review are locally implemented; company/legal-entity and policy setup remain required. Production Keycloak database/backups, least-privilege service-account configuration, HTTPS, durable email delivery/reconciliation, abuse protection, secrets, availability and security review remain unapproved. `AUTH_MODE=disabled` and `ACCOUNT_PROVISIONING_MODE=disabled` stay the defaults.
