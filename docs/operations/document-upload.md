...
This path is disabled by default and rejects `local_test` when `NODE_ENV=production`. It must not receive customer documents. No production object storage, authorized download, file backup/restore or retention controls exist yet.
