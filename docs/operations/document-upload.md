...
For clean, unexpired documents, the same authorized owner/HR role can request `GET /api/v1/tenants/{tenantId}/employees/{employeeId}/documents/{documentId}/content`. The API checks access on every request and verifies the stored size and digest before serving bytes as an attachment. It does not issue a reusable storage URL. Employee self-service download is not implemented.
