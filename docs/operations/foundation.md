...
The membership and invitation migrations add global `identities`, tenant `memberships` and tenant `owner_invitations` with forced RLS. `kinto_app` can use only constrained control-owner functions for initial-owner reconciliation, delivery state and exact-subject activation; it has no ordinary unrestricted write path. Do not manually provision real customers: the optional [Keycloak adapter](authentication.md) is disabled by default and is only locally verified. `inAuthorizedTenant` remains the separate business-authorization boundary. The recovery drill restores and verifies identity, membership and invitation data with existing business tables; never restore old authentication Redis sessions alongside recovered business data.
