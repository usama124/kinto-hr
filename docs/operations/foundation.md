...
The membership-access migration adds global `identities` and tenant `memberships` with forced RLS. `kinto_app` has SELECT only, scoped by transaction-local verified issuer/subject and tenant. Bootstrap grants these reads but creates no identity, owner or membership. Do not manually provision real customers: protected first-owner/invitation flows are not implemented. Local real-provider MFA/reset/back-channel behavior is verified but is not production identity approval. The optional [OIDC/session adapter](authentication.md) accepts existing identities only and is disabled by default. `inAuthorizedTenant` remains the separate business-authorization boundary. The recovery drill now restores and verifies identity/membership data as well as existing business tables; never restore old authentication Redis sessions alongside recovered business data.
