...
The membership and invitation migrations add global `identities`, tenant `memberships`, tenant `owner_invitations` and tenant `employee_account_requests` with forced RLS. `kinto_app` can use only constrained control-owner functions for company/employee requests, initial-owner reconciliation, delivery state and exact-subject activation; it has no ordinary unrestricted write path to these tables. Employee requests create no identity or membership. Do not manually provision real customers: the optional [Keycloak adapter](authentication.md) is disabled by default and is only locally verified for initial owners. `inAuthorizedTenant` remains the separate business-authorization boundary. The recovery drill restores and verifies identity, membership and both invitation/request records with existing business tables; never restore old authentication Redis sessions alongside recovered business data.
