...
`POST /api/v1/tenants/{tenantId}/employees/{employeeId}/account-invitations` requires a current server session, exact same-origin CSRF, a UUID idempotency key, recent trusted MFA and an active owner or `hr_admin` membership in that tenant. It accepts only an email and records one request for an existing draft/active employee. When provisioning is disabled, `202 pending_identity_provider` grants no access. Under the reviewed opt-in, the same Keycloak adapter reconciles the exact account, persists an employee invitation, requests setup delivery and returns its pending state. Exact-identity trusted MFA atomically creates one fixed `employee` membership and employee/identity link. Existing same-company membership, wrong identity, expiry, replay or provider failure grants none.
...
The first-operator, atomic company request, Keycloak initial-owner activation and employee request/activation boundaries are implemented; see [company-request evidence](../evidence/phase-01/company-provisioning.md), [owner-invitation evidence](../evidence/phase-01/owner-invitations.md), [employee-account request evidence](../evidence/phase-01/employee-account-requests.md) and [employee activation evidence](../evidence/phase-01/employee-account-activation.md). Next: membership administration, last-owner protection and customer security-audit access. Provider delivery failure/reconciliation and identity-disable synchronization remain production recovery gates. Company/policy administration and employee CRUD APIs remain closed until their authorization/lifecycle requirements are complete. See the [roadmap](../implementation/README.md).
