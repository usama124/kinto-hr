...
The integration suite covers missing context, cross-tenant reads/writes and audit cursors, pooled transactions/rollback, competing final-seat activations, event-write failure, stale activation, suspended tenants, runtime-role safety, immutable audit permissions and effective-dated organization policy history. Browser tests cover real readiness, route navigation, mobile overflow, error/retry, invalid health responses, owner audit review and the legal-employer/branch/policy workflow. Test evidence is uploaded by CI.
...
- `apps/api`: NestJS health/authentication endpoints, server-side tenant selection, protected account requests, owner membership administration, tenant security-audit access, organization/policy administration, and safe HTTP boundary.
...
Next: implement P01-03 immutable plans, tenant subscriptions, complimentary grants and authoritative capacity resolution. The scoped P01-02 application boundary, including legal employer, branches and effective-dated organization defaults, is locally verified. Initial-owner, employee and administrator provider activation still need durable delivery/reconciliation and production approval. Signed provider back-channel revocation is locally verified; identity-disable synchronization and incident-wide revocation remain operational gates. The Keycloak LoA 2 flow remains the only trusted MFA profile. Local database restore and worker monitoring are implemented; scheduled encrypted backups/PITR, private-file recovery, alert notification delivery, staging provisioning and remote CI success remain unverified. Foundation/Phase 1 is not complete: immutable entitlements and full employee/compensation/import workflows are still pending. Real K50/firmware validation, independently reviewed Pakistan payroll rules, hosting/privacy and commercial approvals remain external release gates. Do not use this preview for live HR/payroll data.
