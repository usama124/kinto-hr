...
- Separate platform-operator authority, guarded first-operator bootstrap, and an MFA/CSRF-protected idempotent company plus pending-owner request endpoint with transactional audit.
...
The employee persistence primitives are a foundation spike, not the complete employee lifecycle, and no employee endpoint is exposed yet. The one control-plane creation endpoint records a company and pending owner request but grants no membership until later provider/invitation success. There is no demo login or authorization bypass. The worker's first consumer only records receipt of committed activation events; business automation is not enabled.

Production recovery hardening, owner invitation activation, employee-account provisioning, employee workflows, imports/documents, attendance, payroll, billing and real K50 communication remain future slices. Payroll permission tests assert a future invariant; they do not mean payroll is available. No customer records or biometric templates are included. See [authentication setup and limitations](docs/operations/authentication.md) and [Keycloak verification](docs/operations/keycloak.md).

Required account access: only platform admins create companies and initial owners; only company admins/authorized HR create employee accounts within their company. Neither companies nor employees can self-register, including on Free plans. The login adapter accepts existing provisioned identities only. Platform admins can now record an idempotent company/pending-owner request, but provider creation, activation and owner membership remain pending. Provider password reset with sign-out-other-devices selected revokes indexed Kinto sessions locally; production recovery/outage approval is still pending. See [account provisioning requirements](docs/implementation/DECISIONS.md).
...
- `apps/api`: NestJS health/authentication endpoints, protected platform provisioning request, and safe HTTP boundary.
