...
- `apps/api`: NestJS health/authentication endpoints, server-side tenant selection, protected company/employee/administrator account requests, owner membership administration, tenant security-audit access, and safe HTTP boundary.
...
Next: implement scoped P01-02 company/legal-entity, branch and effective-dated policy administration before employee CRUD APIs. Customer-visible security-audit access, tenant discovery/selection, administrator invitations, owner membership mutation and last-owner protection are locally verified. Initial-owner, employee and administrator provider activation still need durable delivery/reconciliation and production approval. Signed provider back-channel revocation is locally verified; identity-disable synchronization and incident-wide revocation remain operational gates. The Keycloak LoA 2 flow remains the only trusted MFA profile. Local database restore and worker monitoring are implemented; scheduled encrypted backups/PITR, private-file recovery, alert notification delivery, staging provisioning and remote CI success remain unverified. Foundation/Phase 1 is not complete: company policies, immutable entitlements and full employee/compensation/import workflows are still pending. Real K50/firmware validation, independently reviewed Pakistan payroll rules, hosting/privacy and commercial approvals remain external release gates. Do not use this preview for live HR/payroll data.
