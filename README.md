...
- Tenant-scoped owner/HR employee-account requests with recent MFA, fixed employee-role intent, idempotency, audit and no access before provider activation.
...
The employee persistence primitives are a foundation spike, not the complete employee lifecycle. The only employee-scoped HTTP mutation records a pending account request for an existing employee; it does not expose employee CRUD or grant access. The control-plane endpoint can record a company and pending owner request; when the separately configured Keycloak adapter succeeds, only the exact invited identity completing trusted MFA can atomically receive the first owner membership. There is no demo login or authorization bypass. The worker's first consumer only records receipt of committed activation events; business automation is not enabled.
