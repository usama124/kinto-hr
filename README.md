...
The HR-facing employee boundary currently supports public draft records, roster/detail reads, profile corrections, dated organization assignments and explicit subscription-capacity-backed activation. Termination, archive, rehire, private details, compensation and checklists remain incomplete. When the separately configured Keycloak adapter succeeds, only the exact reconciled identity completing trusted MFA can atomically receive the fixed employee membership and durable employee link. The same boundary grants the exact invited first owner for a platform-created company. There is no demo login or authorization bypass. The worker's first consumer only records receipt of committed activation events; business automation is not enabled.
