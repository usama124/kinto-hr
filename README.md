...
- Owner/HR employee roster, complete monthly-salaried draft creation, capacity-backed activation and rehire with tenant-scoped organization references, retained employment periods, cycle-safe effective-dated reporting assignments, versioned public profile edits and separately loaded/versioned private contact, emergency, address and CNIC details. Private values stay outside roster/audit/outbox projections. Effective-dated PKR compensation history is available only to explicit payroll roles; bank data remains unavailable.
...
The HR-facing employee boundary currently supports public draft records, roster/detail reads, profile corrections, dated organization assignments, explicit subscription-capacity-backed activation, scheduled termination with effective-time access revocation, non-destructive archive and capacity-backed rehire into a new employment period. Rehire does not silently restore revoked login access. Restricted contact, emergency-contact, address and CNIC details are separate from public projections. Effective-dated compensation is permission-separated; onboarding checklists remain incomplete. When the separately configured Keycloak adapter succeeds, only the exact reconciled identity completing trusted MFA can atomically receive the fixed employee membership and durable employee link. The same boundary grants the exact invited first owner for a platform-created company. There is no demo login or authorization bypass. The restricted dispatcher applies bounded due terminations; the worker observer records committed employee lifecycle events without sending email or performing payroll work.

Production invitation reconciliation hardening, checklist workflows, imports/documents, attendance, payroll, billing and real K50 communication remain future slices. Dated entitlement grants and employee-limit overrides are locally implemented, but the plan seeds have no prices and cannot issue invoices. The organization policy framework currently supports only the implemented default-branch setting; attendance, leave and payroll rules are deliberately unavailable. Payroll permission tests assert a future invariant; they do not mean payroll is available. No customer records or biometric templates are included. See [authentication setup and limitations](docs/operations/authentication.md) and [Keycloak verification](docs/operations/keycloak.md).
