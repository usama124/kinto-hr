...
- Retry-safe employee-document metadata registration with opaque quarantine keys, strict PDF/JPEG/PNG and 10 MB bounds, tenant isolation, and an off-by-default private local-test upload/ClamAV scan path. Clean-only audited HR and employee-visible self-service downloads are available in local-test mode. A read-only self profile exposes only own core identity and contact/emergency fields.
