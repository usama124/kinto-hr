...
- Recent-MFA owner membership listing, versioned administrative role changes and revocation with mandatory audit reasons, employee-link protection and an atomic last-owner guard.
- Owner-created administrator invitations with explicit roles, idempotent provider setup delivery and exact-identity MFA activation; HR cannot grant tenant authority.
- Active-company discovery and server-side workspace selection with automatic single-company selection, CSRF rotation, stale-access clearing and selected-tenant enforcement on every tenant-scoped HTTP route.
- Owner-only, recent-MFA tenant security-audit access with safe projections, strict filters, opaque keyset pagination and a responsive customer review screen; platform-operator events remain separate.
- One Pakistan legal employer per tenant, multiple branches, and typed effective-dated organization-default policy drafts with owner preview/publication, immutable history, audit/outbox records and an owner/HR company-setup screen.
- Owner/HR employee roster, complete monthly-salaried draft creation, capacity-backed activation and rehire with tenant-scoped organization references, retained employment periods, cycle-safe effective-dated reporting assignments, versioned public profile edits and separately loaded/versioned private contact, emergency, address and CNIC details. Basic onboarding/offboarding tasks retain assignee, due date and completion evidence; termination scheduling creates a final-settlement review task. Private values stay outside roster/audit/outbox projections. Effective-dated PKR compensation history is available only to explicit payroll roles; bank data remains unavailable.
- Immutable price-free plan versions for Free 5, Starter 20, Growth 50, Business 100 and Scale 250; platform provisioning creates a tenant base subscription, subscribed capacity is enforced under the tenant lock, and owner/HR can review the effective plan and usage.
- Unit/API, real PostgreSQL and desktop/mobile browser regression suites; coverage gates and GitHub Actions CI.
- Digest-backed, row-numbered employee CSV validation with a fixed downloadable template and retry-safe atomic confirmation; raw CSV is not retained, and invalid or over-capacity batches create no employees.
- Retry-safe employee-document metadata registration with opaque quarantine keys, strict PDF/JPEG/PNG and 10 MB bounds, tenant isolation, and an off-by-default private local-test upload/ClamAV scan path. Clean-only audited HR and employee-visible self-service downloads are available in local-test mode. A read-only self profile exposes only own core identity and contact/emergency fields; employees can submit pending contact-change proposals without altering approved details.
- The complete [implementation roadmap](docs/implementation/README.md) and six phase specifications.

The HR-facing employee boundary currently supports public draft records, roster/detail reads, profile corrections, dated organization assignments, explicit subscription-capacity-backed activation, scheduled termination with effective-time access revocation, non-destructive archive, capacity-backed rehire into a new employment period and basic employment-period checklists. Rehire does not silently restore revoked login access. Restricted contact, emergency-contact, address and CNIC details are separate from public projections. Effective-dated compensation is permission-separated. When the separately configured Keycloak adapter succeeds, only the exact reconciled identity completing trusted MFA can atomically receive the fixed employee membership and durable employee link. The same boundary grants the exact invited first owner for a platform-created company. There is no demo login or authorization bypass. The restricted dispatcher applies bounded due terminations; the worker observer records committed employee lifecycle events without sending email or performing payroll work.

Production invitation reconciliation hardening, reusable checklist templates/notifications, production document object storage/scanner operations/download, attendance, payroll, billing and real K50 communication remain future slices. Dated entitlement grants and employee-limit overrides are locally implemented, but the plan seeds have no prices and cannot issue invoices. The organization policy framework currently supports only the implemented default-branch setting; attendance, leave and payroll rules are deliberately unavailable. Payroll permission tests assert a future invariant; they do not mean payroll is available. No customer records or biometric templates are included. See [authentication setup and limitations](docs/operations/authentication.md) and [Keycloak verification](docs/operations/keycloak.md).

Required account access: only platform admins create companies and initial owners; only company admins/authorized HR request employee accounts within their company. Neither companies nor employees can self-register, including on Free plans. With provisioning disabled, login accepts existing identities only. The optional reviewed Keycloak path creates or exactly reconciles an approved initial owner, employee or invited administrator, sends provider-managed setup actions, and grants only the roles fixed by the corresponding request after exact-subject trusted MFA. Provider password reset with sign-out-other-devices selected revokes indexed Kinto sessions locally; production delivery/recovery/outage approval is still pending. See [account provisioning requirements](docs/implementation/DECISIONS.md).
...
The integration suite covers missing context, cross-tenant reads/writes and audit cursors, pooled transactions/rollback, subscribed final-seat races, event-write failure, stale activation, suspended tenants, runtime-role safety, immutable audit/plan records and effective-dated organization policy history. Browser tests cover real readiness, route navigation, mobile overflow, error/retry, invalid health responses, owner audit review, company setup and effective plan/capacity review. Test evidence is uploaded by CI.
...
- `apps/api`: NestJS health/authentication endpoints, server-side tenant selection, protected account requests, owner membership administration, tenant security-audit access, organization/policy/catalog and employee-record administration, effective entitlements, and safe HTTP boundary.
...
Next: implement production object-store transfer, scanner operations and file recovery after the [local-test employee document upload/scan/download boundary](docs/evidence/phase-01/employee-documents.md) and [employee import validation workflow](docs/evidence/phase-01/employee-import-preview.md). P01-04 is locally implemented through restricted employee details, [effective-dated compensation](docs/evidence/phase-01/employee-compensation.md), lifecycle/rehire and [basic checklists](docs/evidence/phase-01/employee-checklists.md). P01-03 remains locally complete. Initial-owner, employee and administrator provider activation still need durable delivery/reconciliation and production approval. Signed provider back-channel revocation is locally verified; identity-disable synchronization and explicit post-rehire account reactivation remain operational gates. Local database restore and worker monitoring are implemented; scheduled encrypted backups/PITR, private-file recovery, alert notification delivery, staging provisioning and remote CI success remain unverified. Foundation/Phase 1 is not complete: production document transfer/scanning/download, file recovery, HR decisions on contact-change requests and reports are still pending. Real K50/firmware validation, independently reviewed Pakistan payroll rules, hosting/privacy and commercial approvals remain external release gates. Do not use this preview for live HR/payroll data.
