...
- Retry-safe employee-document metadata registration with opaque quarantine keys, strict PDF/JPEG/PNG and 10 MB bounds, tenant isolation, and an off-by-default private local-test upload/ClamAV scan path. A clean-only, audited HR download path is available in local-test mode.
