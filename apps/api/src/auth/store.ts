...
export class AuthStore {
  ...
  constructor(
    url: string,
    private readonly prefix = 'kinto:auth:v1:',
  ) {
  ...
  async createSession(
    data: Pick<Session, 'principal' | 'identityId' | 'authTime'>,
    oldToken?: string,
  ) {
    ...
    const raw = await this.redis.eval(
      ...
      if ARGV[4] == 'yes' then redis.call('DEL', KEYS[2]) end
  ...
  async deleteSession(token: string) {
    await this.redis.del(this.key('session', token));
  }
}
